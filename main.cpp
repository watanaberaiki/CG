...
//デバッグレイヤーをオンに
	...
	// 値を書き込むと自動的に転送される
	constMapMaterial->color = XMFLOAT4(1, 0, 0, 1.0f);              // RGBAで半透明の赤
...
//}
	...
	while (true) {
		...
		if (key[DIK_Z] || key[DIK_C]) {
			if (key[DIK_Z] && alfa < 1.00f) {
				alfa += 0.01f;
			}
			else if (key[DIK_C] && alfa > 0.00f) {
				alfa -= 0.01f;
			}

		}
		constMapMaterial->color = XMFLOAT4(0 + change, 0 + change, 0, alfa);
